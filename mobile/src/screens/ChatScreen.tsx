...
import { getChatMessages, sendChatMessage, markMessagesAsRead, getTherapySession, getTherapySessionByRequest, ChatMessage, TherapySession, createSkyroomRoom, getSkyroomRoom, createSkyroomLoginUrl } from '../api/client';
...
export default function ChatScreen({ route, navigation }: ChatScreenProps) {
	...
	const isMessageMine = (message: ChatMessage): boolean => {
		...
		// Use current_user_id from session if available, otherwise fallback to patient
		const currentUserId = session.current_user_id || session.patient;
	...
	const loadMessages = async () => {
		...
		try {
			...
			// Recalculate is_mine based on current user (patient)
	...
	// Determine the other user's name (for patient, show therapist name, and vice versa)
	const currentUserId = session.current_user_id || session.patient;
	const isGroupTherapy = session.is_group_therapy || false;
	const otherUserName = isGroupTherapy 
		? `گروه درمانی (${session.patients_count || 0} بیمار)`
		: (session.therapist_name || 'درمانگر');
	const otherUserPhone = session.therapist_phone || '';
	const currentUserPhone = session.patient_phone || '';
	const therapistImageUrl = session.therapist_profile_image_url
		? getBucketFileUrl(session.therapist_profile_image_url)
		: null;
	...
	const handleVideoCall = async () => {
		...
		try {
			...
			// Create login URL with presenter access for full camera/microphone access
			const loginUrl = await createSkyroomLoginUrl({
				room_id: roomId,
				user_id: `patient-${session.patient}`,
				nickname: userName,
				access: 2, // Presenter access - allows full camera and microphone access
				language: 'fa',
				ttl: 3600, // 1 hour
			});
			...
			<View style={styles.header}>
				...
				{therapistImageUrl ? (
					<TouchableOpacity
						activeOpacity={0.85}
						onPress={() => {
							setSelectedTherapistId(session.therapist_profile_id ?? null);
							setSelectedInitialProfile({ full_name: session.therapist_name, profile_image_url: session.therapist_profile_image_url });
							setModalVisible(true);
						}}
					>
						<Image source={{ uri: therapistImageUrl }} style={styles.headerAvatar} />
					</TouchableOpacity>
				) : null}
				<View style={styles.headerInfo}>
					<Text style={styles.headerTitle}>جلسه درمانی</Text>
					<Text style={styles.headerSubtitle}>{otherUserName}</Text>
					{isGroupTherapy && session.patients_list && session.patients_list.length > 0 ? (
						<View style={styles.participantsContainer}>
							<Text style={styles.participantsLabel}>شرکت‌کنندگان:</Text>
							{session.patients_list.map((patient, idx) => (
								<Text key={patient.id} style={styles.participantName}>
									{idx + 1}. {patient.name || patient.phone}
									{patient.id === currentUserId ? ' (شما)' : ''}
								</Text>
							))}
							<Text style={styles.participantName}>
								{session.patients_list.length + 1}. {session.therapist_name || 'درمانگر'}
							</Text>
						</View>
					) : (
						<>
				...
				>
					{messages.length === 0 ? (
						...
						messages.map((message, index) => {
							...
							const showSenderName = isGroupTherapy && !isMyMessage;
							return (
							<View
